...
void add_position(int from_height,int to_height,int max_height, positions new_positions, int *num_new_position , int itching[] , board min_moves)
{
    if(to_height<=max_height && itching[to_height]==0 && min_moves[to_height] == -1){
        min_moves[to_height]=1+min_moves[from_height];
        new_positions[*num_new_position] = to_height;
        (*num_new_position)++;
    }
}
...
void solve(int target_height, board min_moves)
{
    ...
    {
        if(min_moves[1]!=-1 && (best == -1 || min_moves[i]<best))
            best = min_moves[i];
    }
    printf("%d\n",best);
}
...
int main(void)
{
    int target_height,jump_distance,num_itching_section;
    static int itching[SIZE*2]={0};
    static board min_moves ;
    int i,j,itch_start,itch_end;
    scanf("%d%d%d",&target_height,&jump_distance,&num_itching_section);
    for(i=0;i<num_itching_section;i++){
