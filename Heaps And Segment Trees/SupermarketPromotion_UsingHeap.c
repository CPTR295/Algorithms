...
int main(void)
{
    ...
    static struct heap_element max_heap[MAX_RECEIPTS];
    static struct heap_element min_heap[MAX_RECEIPTS];
    ...
    for(i=0;i<num_days;i++)
    {
        ...
        while(used[max_element.receipt_index])
            max_element = max_heap_extract(max_heap, &max_num_heap);
        used[max_element.receipt_index] = 1; // Mark the receipt as used
        
        min_element = min_heap_extract(min_heap, &min_num_heap);
        while(used[min_element.receipt_index])
            min_element = min_heap_extract(min_heap, &min_num_heap);
        used[min_element.receipt_index] = 1; // Mark the receipt as used
        total_price += max_element.cost - min_element.cost;
