# 🍔 Burger Fervor Problem — Recursive Optimization

## 📌 Problem Statement

Homer Simpson has **t minutes** to spend eating **burgers** and drinking **beer**.

There are **two types of burgers**:

* First burger takes **m minutes**
* Second burger takes **n minutes**

Homer prefers **burgers over beer**, so he wants to:

* Spend **as much time as possible eating burgers**
* If exact time **t** is not possible, use leftover time for **beer**

Your task is to determine:

* Maximum number of burgers Homer can eat
* Remaining time (if any)

---

## 🧠 Example

### Example 1

Input:

```text
4 9 15
```

Output:

```text
3 3
```

Explanation:

* Best possible: 3 burgers (4 + 4 + 4 = 12 minutes)
* Remaining time = 3 minutes (beer)

---

### Example 2

Input:

```text
3 5 15
```

Output:

```text
5
```

Explanation:

* 3 + 3 + 3 + 3 + 3 = 15 minutes
* No beer needed

---

## 📥 Input

* Multiple test cases
* Each line contains:

```text
m n t
```

Where:

* `m` → Time for first burger
* `n` → Time for second burger
* `t` → Total available time

### Constraints

| Parameter           | Limit    |
| ------------------- | -------- |
| m, n, t             | < 10,000 |
| Multiple test cases | Yes      |

---

## 📤 Output

For each test case:

### Case 1 — Exact Match

```text
<number_of_burgers>
```

### Case 2 — Not Exact Match

```text
<number_of_burgers> <remaining_time>
```

---

## ⚙️ Approach

This solution uses:

* Recursion
* Backtracking
* Optimization

### Steps

1. Try eating burger **m**
2. Try eating burger **n**
3. Choose maximum burgers
4. If exact match not possible:

   * Reduce time
   * Find best possible combination

---

## 🔑 Key Functions

### `solve_t()`

* Recursive function
* Finds maximum burgers for given time

```c
int solve_t(int m, int n, int t)
```

---

### `solve()`

* Handles output
* Calculates remaining beer time

---

### `max()`

* Returns maximum value

---

## 🧠 Algorithm Type

* Recursion
* Dynamic Optimization
* Backtracking

---

## ⏱️ Time Complexity

| Operation       | Complexity                      |
| --------------- | ------------------------------- |
| Recursive calls | O(t) worst                      |
| Overall         | Efficient for given constraints |

