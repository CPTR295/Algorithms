...
void find_distance(edge *adj_list[], int person, int num_people, int min_moves[])
{
    static int cur_positions[MAX_PEOPLE], new_positions[MAX_PEOPLE];
...
int main(void)
{
    static edge *adj_list[MAX_PEOPLE]={NULL};
    static int min_moves[MAX_PEOPLE];
    ...
    for(i=0;i<num_ops;i++){
        ...
        if(op=='A'){
            ...
            find_distance(adj_list,person1,num_people,min_moves);
            size1=size(num_people,min_moves);
            same_community=0;
            if(min_moves[person2]!=-1){
                same_community=1;
            }
            find_distance(adj_list,person2,num_people,min_moves);
